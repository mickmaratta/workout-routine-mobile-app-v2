...
const AddExerciseScreen = ({ route, navigation }) => {
  ...
  const [sets, setSets] = useState([{ number: 1, reps: null, weight: null }]);
    ...
    <DismissKeyboard>
      <View className="flex-1">
        ...
        <View className="mx-3 space-y-10">
          <TextInput onChangeText={(text) => setTitle(text)} placeholder="Exercise Name" className="text-2xl" />
        ...
        <View className="my-8">
          {sets.map((set) => (
            <AddSet key={set.number} set={set} handleReps={handleReps} handleWeight={handleWeight} />
          ))}
        </View>
        <View className="items-center flex-row justify-center space-x-2">
        <TouchableOpacity onPress={addExercise} className="bg-red-900 w-36 justify-center items-center rounded-md">
              <Text className="py-3 text-gray-100 text-lg">Cancel</Text>
            </TouchableOpacity>
          <TouchableOpacity onPress={addExercise} className="bg-sky-900 w-36 justify-center items-center rounded-md">
